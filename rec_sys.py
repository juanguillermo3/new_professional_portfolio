...
class RecommendationSystem:
    ...
    def render_project_metadata(self, project_metadata):
        """Render project title, description, tags, milestones, and video with milestones in an expandable section."""
        video_filename = f"{project_metadata['title'].replace(' ', '_').lower()}_theme.mp4"
        video_path = os.path.join('assets', video_filename)
        ...
        # Render milestones in an expandable section
        with st.expander("Project Milestones"):
            if 'achieved_milestones' in project_metadata and 'next_milestones' in project_metadata:
                milestone_html = ''.join(
                    [f'<div style="color:green; font-size:95%; font-weight:normal;">🟢 {m}</div>' for m in project_metadata['achieved_milestones']]
                ) + ''.join(
                    [f'<div style="color:yellow; font-size:95%; font-weight:normal;">🟡 {m}</div>' for m in project_metadata['next_milestones']]
                )
                st.markdown(
                    f"""
                    <div style="width:80%; margin:auto; background-color:rgb(224, 224, 224); padding:5px;">
                        {milestone_html}
                    </div>
                    """,
                    unsafe_allow_html=True,
