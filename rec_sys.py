...
from front_end_for_recommended_content import html_for_item_data, html_for_milestones_from_project_metadata, get_hashed_id
...
class RecommendationSystem(PortfolioSection):
    ...
    def render_card(self, rec, **kwargs):
        """Render a single recommendation card with dynamic HTML generation, buttons, and tooltips."""
    
        # Generate a unique hash ID for the item
        unique_id = get_hashed_id(rec.get("title", "Untitled"), rec.get("repo_name", "default_repo"))
    
        # Render title box (without description)
        st.markdown(html_for_item_data(rec), unsafe_allow_html=True)
    
        # Get description for tooltip (defaulting to a placeholder if missing)
        description_text = rec.get("description", "No additional details available.")
    
        # Apply tooltip to the title box with the description
        apply_custom_tooltip(unique_id, description_text)
    
        # Unique button ID for Galleria
        button_id = f"galleria_{unique_id}"
    
        # Render Galleria button if 'image_path' exists
        if "image_path" in rec:
            st.markdown(
                """
                <style>
            ...
            # Apply tooltip to Galleria button
            apply_custom_tooltip(button_id, "View the image gallery for this item")
        ...
        # Create button columns if needed
        button_cols = st.columns(len(buttons)) if buttons else []
    
        for col, (label, url, color) in zip(button_cols, buttons):
            with col:
                btn_html = render_external_link_button(url, label, color)
                st.markdown(btn_html, unsafe_allow_html=True)
                apply_custom_tooltip(label.lower().replace(" ", "_"), f"Open {label} in a new tab")
