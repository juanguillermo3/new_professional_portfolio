...
from front_end_utils import render_section_separator, render_external_link_button, prettify_title, tags_in_twitter_style, render_external_link_button_as_train_ticket
...
class RecommendationSystem(PortfolioSection):
    ...
    def _render_control_panel(self):
        """Render the control panel with sticky positioning inside its container."""
        ...
        with st.container():
            ...
            st.markdown('</div>', unsafe_allow_html=True)  # Close the control-panel div
            st.markdown('</div>', unsafe_allow_html=True)  # Close the control-container div
