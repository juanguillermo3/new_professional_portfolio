...
class HeroArea:
    ...
    def render_detailed_offering(self, id_pattern="offering-{}", colors=["#f0f0f0", "#ffffff"]):
        ...
        label, color, pastel_color, icon_url, emoji, default_text = (
            figure_style["label"], figure_style["color"], figure_style["pastel"], 
            figure_style["icon"], figure_style["emoji"], figure_style["default_text"]
        )
        ...
        for i, offer in enumerate(offerings):
            ...
            expanded_html, expanded_style = expandable_text_html(offer["description"])
            style_accumulator += expanded_style
            ...
            if "skills" in offer:
                ...
                summary = default_text.format(n=skills_count)
                visible_milestone = (
                    f'<div style="color:{color}; text-align: center;">'
                    f'<img src="{icon_url}" alt="{label}" style="width: 30px; height: 30px;"/><br>'
                    f'<label>{summary}</label></div>'
                )
                tooltip_content = "".join(
        ...
        offering_html += '</ul>'
        st.markdown(offering_html, unsafe_allow_html=True)
        st.markdown(style_accumulator, unsafe_allow_html=True)
