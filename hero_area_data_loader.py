...
def load_detailed_offering(id_pattern="offering-{}", colors=["#f0f0f0", "#ffffff"]):
    ...
    for i, offer in enumerate(offerings):
        ...
        # Insert the tooltip for the list of technical skills
        if "skills" in offer:
            offering_html +=  html_for_tooltip_from_large_list(offer["skills"], label="Technical Skills", element_id=element_id, color="#555", emoji="🛠️")
    ...
    st.markdown(install_tooltip_styling(), st.markdown(tooltip_html, unsafe_allow_html=True))
    
    # Add tooltip hover logic for all elements
    for element_id in tooltip_elements:
        st.markdown(install_tooltip_triggering_logic(element_id), st.markdown(tooltip_html, unsafe_allow_html=True))
