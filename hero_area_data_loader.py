...
def load_detailed_offering(id_pattern="offering-{}", colors=["#f0f0f0", "#ffffff"]):
    ...
    for i, offer in enumerate(offerings):
        bg_color = colors[i % len(colors)]
        offering_id = id_pattern.format(i+1)

        # Mock hard skills list
        hard_skills = [f"{offer['title']} {j+1}" for j in range(2)]

        # Generate tooltip HTML
        tooltip_html = html_for_tooltip_from_large_list(hard_skills, label=offer["title"], color="#555", emoji="📌")

        offering_html += f'<li id="{offering_id}" style="background-color: {bg_color}; padding: 8px; border-radius: 4px;">'
        offering_html += f'<strong>{i+1}. {tooltip_html}</strong>: {offer["description"]}'
