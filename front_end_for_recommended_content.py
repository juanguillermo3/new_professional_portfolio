...
def html_for_milestones_from_project_metadata(milestones=None, project_metadata=None, milestone_type="achieved_milestones"):
    ...
    # Define milestone properties with optimized contrast and pastel backgrounds
    milestone_labels = {
        "achieved_milestones": ("Achieved Milestones", "#2E7D32", "#A5D6A7", "✅"),  # Dark green & pastel green
        "next_milestones": ("Upcoming Milestones", "#C28F00", "#FFE082", "🚧"),  # Gold-ish yellow & pastel yellow
        "code_samples": ("Code Samples", "#1565C0", "#90CAF9", "💾")  # Deep blue & pastel blue
    }
    
    label, color, bg_color, icon = milestone_labels.get(milestone_type, ("Milestones", "black", "#E0E0E0", "📌"))
    ...
    # Return formatted HTML with refined frosted effect and hover interaction
    return f"""
    <div id="{element_id}-container" style="position: relative; display: inline-block; cursor: pointer;">
        <span id="{element_id}" style="border-bottom: 1px dashed gray; cursor: pointer;" class="hover-trigger">
            {visible_milestone}
        </span>
        <div id="{element_id}-tooltip" style="visibility: hidden; opacity: 0; transform: translateY(5px) scale(0.95); 
            transition: opacity 0.3s ease-in-out, visibility 0.3s ease-in-out, transform 0.3s ease-in-out;
            background-color: rgba(240, 240, 240, 0.7); /* Softer frosted effect */
            backdrop-filter: blur(1px); /* Stronger blur for a glassy look */
            color: black;
    ...
    <style>
        #{element_id}-container:hover {{
            background-color: {bg_color}; /* Pastel counterpart background */
            transition: background-color 0.3s ease-in-out;
        }}
        #{element_id}:hover + #{element_id}-tooltip {{
            visibility: visible;
            opacity: 1;
            transform: translateY(0px) scale(1.1);
        }}
