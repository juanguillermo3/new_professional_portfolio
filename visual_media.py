...
class VisualContentGallery:
    def __init__(self, title, description, media_path, width="700px", height="400px"):
        ...
        self.width = width
        self.height = height
        self.gallery_key = self._generate_gallery_key()

        if f"{self.gallery_key}_index" not in st.session_state:
            st.session_state[f"{self.gallery_key}_index"] = 0

        self.file_list = self._find_media_files(media_path)

    def _generate_gallery_key(self):
        return hashlib.md5(f"{self.title}{self.description}".encode()).hexdigest()
    ...
    def render(self):
        ...
        current_index = st.session_state[f"{self.gallery_key}_index"]
        current_file = self.file_list[current_index]
        self.parse_media(current_file)

        if len(self.file_list) > 1:
            cols = st.columns(len(self.file_list))
            for idx, col in enumerate(cols):
                with col:
                    if st.button(
                        f"{idx + 1}",
                        key=f"{self.gallery_key}_btn_{idx}",
                        help=f"View media {idx + 1}",
                        type="secondary",
                        use_container_width=True
                    ):
                        st.session_state[f"{self.gallery_key}_index"] = idx
                        st.rerun()
