...
def setup_tooltip_behavior(unique_id):
    """
    Injects JavaScript and CSS to ensure the tooltip appears on hover.
    ...
    document.addEventListener("DOMContentLoaded", function() {{
        var trigger = document.getElementById("{unique_id}");
        var tooltip = document.querySelector(".skills_tooltip-{unique_id}");
        
        if (trigger && tooltip) {{
            trigger.addEventListener("mouseover", function() {{
                tooltip.style.visibility = "visible";
                tooltip.style.opacity = "1";
                tooltip.style.transform = "translateY(0px) scale(1.1)";
            }});
    ...
    st.markdown(tooltip_css + tooltip_js, unsafe_allow_html=True)
